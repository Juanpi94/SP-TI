import { updateActivated, loadSuccess, updateDeactivated } from "./recursos/loadUpdateDatas/messages";
		...
		axios.get(`/api/funcionarios/`).then((response) => {
			...
			for (let x = 1; x < rowData.length; x++) {
				...
				if (codigo != undefined && codigo != "" && codigo != null) {
					...
					exclu_uni.push({ posicion: x + 1, coordinacion_invalida: "Coordinación no valida [" + codigo + "]" });
				}
			}
			uni_data.forEach(element => {
				axios.post('/api/coordinaciones/', element, config).catch((err) => {
					console.log(err.response.data);
				});
			});
		}), console.log('Coordinaciones terminado con exito');
		...
		setTimeout(() => {
			axios.get(`/api/instalaciones/`).then((response) => {
				...
				axios.get(`/api/funcionarios/`).then((response) => {
					...
					axios.get(`/api/coordinaciones/`).then((response) => {
						const coordinaciones_list = response.data;
						...
						for (let x = 1; x < rowData.length; x++) {
							...
							if (ubicacion != undefined && ubicacion != "" && ubicacion != null) {
								if (!ubi_data.some(item => format_text(item.ubicacion) === format_text(ubicacion))) {
									...
									for (let i = 0; i < coordinaciones_list.length; i++) {
										if (coordinaciones_list[i].nombre === coordinacion) {
											id_coordinacion = coordinaciones_list[i].codigo;
											break;
										}
									}
									ubi_data.push({
						...
						console.log(ubi_data.ubicacion);
		...
		setTimeout(() => {
			async function fetchData() {
				...
				informe_errores.push({
					...
					'Informe coordinaciones': exclu_uni,
