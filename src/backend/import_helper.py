...
class ImportModule:
    ...
    def import_activos(data: list, update: bool) -> ImportResult:
        ...
        for idx, row in enumerate(data):
            ...
            if not isna(subtipo) and subtipo.strip():
                subtipo_db, _ = models.Subtipo.objects.get_or_create(defaults={"detalle": ""}, nombre=subtipo)
                activo.subtipo = subtipo_db
            if not isna(ubicacion) and ubicacion.strip():
                instalacion = models.Ubicaciones.InstalacionChoices.ESPARZA if zona.lower() == "nances-esparza" else models.Ubicaciones.InstalacionChoices.COCAL
                if not isna(custodio) and custodio.strip():
                    custodio, _ = models.Funcionarios.objects.get_or_create(defaults={"cedula": ""},
                                                                            nombre_completo=custodio)
                    ubicacion_db, _ = models.Ubicaciones.objects.get_or_create(defaults={
            ...
            try:
                ...
                details["failed"] += 1
                details["errors"].append(f"No se pudo guardar el activo en fila {row_number}\n{e.__str__()}")
        return details
    ...
    def import_compras(data: list, update: bool) -> ImportResult:
        ...
        for idx, row in enumerate(data):
            assert len(row) == 11, "Se envió una cantidad incorrecta de datos " + str(len(row))
            row_idx = idx + 2

            [numero_compra, origen_presupuesto, decision, solicitud, procedimiento, factura, proveedor,
             telefono_proveedor, correo_proveedor, detalle, informe] = row
            ...
            if qs.count() > 0 and not update:
                ...
                compra = Compra(numero_orden_compra=numero_compra, origen_presupuesto=origen_presupuesto,
                                decision_inicial=decision, numero_solicitud=solicitud,
                                numero_procedimiento=procedimiento,
                                numero_factura=factura, detalle=detalle, informe_tecnico=informe)

            if not isna(proveedor):
                proveedor, _ = Proveedor.objects.get_or_create(nombre=proveedor, defaults={
            ...
            try:
                ...
                details["failed"] += 1
                details["errors"].append(f"No se pudo guardar la compra en fila {row_idx}\n{e.__str__()}")
        return details
