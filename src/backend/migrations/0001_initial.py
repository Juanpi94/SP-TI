# Generated by Django 4.2.7 on 2024-07-30 21:30

from django.conf import settings
from django.db import migrations, models
import django.db.models.deletion
import django.utils.timezone


class Migration(migrations.Migration):

    initial = True

    dependencies = [
        migrations.swappable_dependency(settings.AUTH_USER_MODEL),
    ]

    operations = [
        migrations.CreateModel(
            name='Activos_No_Plaqueados',
            fields=[
                ('nombre', models.CharField(max_length=120, verbose_name='Nombre')),
                ('valor_colones', models.DecimalField(decimal_places=2, max_digits=12)),
                ('valor_dolares', models.DecimalField(decimal_places=2, max_digits=12)),
                ('garantia', models.DateField(null=True, verbose_name='Garantia')),
                ('fecha_registro', models.DateField(null=True, verbose_name='Fecha de Registro')),
                ('fecha_ingreso', models.DateField(null=True, verbose_name='Fecha de Ingreso')),
                ('observacion', models.CharField(blank=True, max_length=500, null=True, verbose_name='Observación')),
                ('serie', models.CharField(max_length=200, primary_key=True, serialize=False)),
            ],
            options={
                'verbose_name_plural': 'Activos No Plaqueados',
            },
        ),
        migrations.CreateModel(
            name='Activos_Plaqueados',
            fields=[
                ('nombre', models.CharField(max_length=120, verbose_name='Nombre')),
                ('valor_colones', models.DecimalField(decimal_places=2, max_digits=12)),
                ('valor_dolares', models.DecimalField(decimal_places=2, max_digits=12)),
                ('garantia', models.DateField(null=True, verbose_name='Garantia')),
                ('fecha_registro', models.DateField(null=True, verbose_name='Fecha de Registro')),
                ('fecha_ingreso', models.DateField(null=True, verbose_name='Fecha de Ingreso')),
                ('observacion', models.CharField(blank=True, max_length=500, null=True, verbose_name='Observación')),
                ('placa', models.CharField(max_length=20, primary_key=True, serialize=False)),
                ('serie', models.CharField(max_length=200, null=True, unique=True, verbose_name='Serie')),
            ],
            options={
                'verbose_name_plural': 'Activos Plaqueados',
            },
        ),
        migrations.CreateModel(
            name='Categoria',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('nombre', models.CharField(max_length=50, unique=True)),
                ('detalle', models.CharField(blank=True, max_length=50, null=True)),
            ],
            options={
                'verbose_name_plural': 'Categorias',
            },
        ),
        migrations.CreateModel(
            name='Coordinaciones',
            fields=[
                ('codigo', models.CharField(max_length=10, primary_key=True, serialize=False)),
                ('nombre', models.CharField(max_length=120, null=True)),
            ],
            options={
                'verbose_name_plural': 'Coordinaciones',
            },
        ),
        migrations.CreateModel(
            name='DetallePlacaUbicacion',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('detalles_placa', models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='backend.activos_plaqueados')),
            ],
            options={
                'verbose_name_plural': 'Detalle Placa Ubicacion',
            },
        ),
        migrations.CreateModel(
            name='DetalleSerieUbicacion',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('detalles_serie', models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='backend.activos_no_plaqueados')),
            ],
            options={
                'verbose_name_plural': 'Detalle Serie Ubicacion',
            },
        ),
        migrations.CreateModel(
            name='Estados',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('nombre', models.CharField(max_length=20, verbose_name='Nombre')),
                ('descripcion', models.CharField(max_length=100, verbose_name='Descripción')),
            ],
            options={
                'verbose_name_plural': 'Estados',
            },
        ),
        migrations.CreateModel(
            name='Funcionarios',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('cedula', models.CharField(max_length=120)),
                ('nombre_completo', models.CharField(max_length=500, unique=True)),
                ('correo_institucional', models.EmailField(max_length=254, unique=True)),
                ('correo_personal', models.CharField(blank=True, max_length=120, null=True)),
                ('telefono_oficina', models.CharField(blank=True, max_length=120, null=True)),
                ('telefono_personal', models.CharField(blank=True, max_length=120, null=True)),
            ],
            options={
                'verbose_name_plural': 'Funcionarios',
            },
        ),
        migrations.CreateModel(
            name='Instalaciones',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('ubicacion', models.CharField(max_length=120, unique=True)),
            ],
            options={
                'verbose_name_plural': 'Instalaciones',
            },
        ),
        migrations.CreateModel(
            name='Marcas',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('nombre', models.CharField(max_length=120, unique=True)),
            ],
            options={
                'verbose_name_plural': 'Marcas',
            },
        ),
        migrations.CreateModel(
            name='Modelos',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('nombre', models.CharField(max_length=120, unique=True)),
            ],
            options={
                'verbose_name_plural': 'Modelos',
            },
        ),
        migrations.CreateModel(
            name='Partida',
            fields=[
                ('codigo', models.CharField(max_length=50, primary_key=True, serialize=False)),
                ('descripcion', models.CharField(blank=True, max_length=50, null=True)),
            ],
        ),
        migrations.CreateModel(
            name='Permissions',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
            ],
            options={
                'verbose_name_plural': 'Permissions',
            },
        ),
        migrations.CreateModel(
            name='Proveedor',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('nombre', models.CharField(max_length=230, unique=True)),
                ('telefono', models.CharField(max_length=16)),
                ('correo', models.CharField(max_length=120)),
            ],
            options={
                'verbose_name_plural': 'Proveedores',
            },
        ),
        migrations.CreateModel(
            name='Red',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('MAC', models.CharField(max_length=45, unique=True)),
                ('IP', models.CharField(blank=True, default='', max_length=20)),
                ('IP6', models.CharField(blank=True, default='', max_length=40)),
                ('IP_switch', models.CharField(blank=True, default='', max_length=80)),
            ],
            options={
                'verbose_name_plural': 'Red',
            },
        ),
        migrations.CreateModel(
            name='Subtipo',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('nombre', models.CharField(max_length=120, unique=True)),
                ('detalle', models.CharField(blank=True, max_length=200)),
            ],
            options={
                'verbose_name_plural': 'Subtipos',
            },
        ),
        migrations.CreateModel(
            name='Tipo',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('nombre', models.CharField(max_length=120, unique=True)),
                ('detalle', models.CharField(blank=True, max_length=200)),
            ],
            options={
                'verbose_name_plural': 'Tipos',
            },
        ),
        migrations.CreateModel(
            name='TiposEstados',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('nombre', models.CharField(max_length=20, unique=True)),
            ],
            options={
                'verbose_name_plural': 'TiposEstados',
            },
        ),
        migrations.CreateModel(
            name='TiposTramites',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('nombre', models.CharField(max_length=20, unique=True)),
            ],
            options={
                'verbose_name_plural': 'TiposTramites',
            },
        ),
        migrations.CreateModel(
            name='Ubicaciones',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('ubicacion', models.CharField(max_length=120, unique=True)),
                ('coordinacion', models.ForeignKey(null=True, on_delete=django.db.models.deletion.SET_NULL, to='backend.coordinaciones')),
                ('custodio', models.ForeignKey(on_delete=django.db.models.deletion.DO_NOTHING, to='backend.funcionarios')),
                ('instalacion', models.ForeignKey(on_delete=django.db.models.deletion.DO_NOTHING, to='backend.instalaciones')),
            ],
            options={
                'verbose_name_plural': 'Ubicaciones',
            },
        ),
        migrations.CreateModel(
            name='Tramites',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('referencia', models.CharField(max_length=120, unique=True)),
                ('detalles', models.CharField(max_length=200)),
                ('obervaciones', models.CharField(max_length=500, null=True)),
                ('fecha', models.DateField(default=django.utils.timezone.now)),
                ('destinatario', models.ForeignKey(null=True, on_delete=django.db.models.deletion.DO_NOTHING, related_name='+', to='backend.funcionarios')),
                ('detalles_placa', models.ManyToManyField(related_name='detalles_placa', through='backend.DetallePlacaUbicacion', to='backend.activos_plaqueados')),
                ('detalles_serie', models.ManyToManyField(related_name='detalles_serie', through='backend.DetalleSerieUbicacion', to='backend.activos_no_plaqueados')),
                ('elaborado_por', models.ForeignKey(on_delete=django.db.models.deletion.DO_NOTHING, to=settings.AUTH_USER_MODEL)),
                ('estado', models.ForeignKey(on_delete=django.db.models.deletion.DO_NOTHING, to='backend.tiposestados')),
                ('remitente', models.ForeignKey(null=True, on_delete=django.db.models.deletion.DO_NOTHING, related_name='+', to='backend.funcionarios')),
                ('tipo', models.ForeignKey(on_delete=django.db.models.deletion.DO_NOTHING, to='backend.tipostramites')),
            ],
            options={
                'verbose_name_plural': 'Tramites',
            },
        ),
        migrations.CreateModel(
            name='Taller',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('destinatario', models.CharField(max_length=200)),
                ('beneficiario', models.CharField(max_length=200)),
                ('autor', models.CharField(max_length=200)),
                ('tramite', models.OneToOneField(on_delete=django.db.models.deletion.CASCADE, related_name='taller', to='backend.tramites')),
            ],
            options={
                'verbose_name_plural': 'Talleres',
            },
        ),
        migrations.AddField(
            model_name='detalleserieubicacion',
            name='tramite',
            field=models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='backend.tramites'),
        ),
        migrations.AddField(
            model_name='detalleserieubicacion',
            name='ubicacion_actual',
            field=models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, related_name='ubicacion_actual_serie', to='backend.ubicaciones'),
        ),
        migrations.AddField(
            model_name='detalleserieubicacion',
            name='ubicacion_futura',
            field=models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, related_name='ubicacion_futura_serie', to='backend.ubicaciones'),
        ),
        migrations.AddField(
            model_name='detalleplacaubicacion',
            name='tramite',
            field=models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='backend.tramites'),
        ),
        migrations.AddField(
            model_name='detalleplacaubicacion',
            name='ubicacion_actual',
            field=models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, related_name='ubicacion_actual_placa', to='backend.ubicaciones'),
        ),
        migrations.AddField(
            model_name='detalleplacaubicacion',
            name='ubicacion_futura',
            field=models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, related_name='ubicacion_futura_placa', to='backend.ubicaciones'),
        ),
        migrations.CreateModel(
            name='Desecho',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('tramite', models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='backend.tramites')),
            ],
            options={
                'verbose_name_plural': 'Desechos',
            },
        ),
        migrations.AddField(
            model_name='coordinaciones',
            name='coordinador',
            field=models.ForeignKey(null=True, on_delete=django.db.models.deletion.SET_NULL, to='backend.funcionarios'),
        ),
        migrations.CreateModel(
            name='Compra',
            fields=[
                ('numero_orden_compra', models.CharField(max_length=40, primary_key=True, serialize=False)),
                ('numero_solicitud', models.CharField(blank=True, max_length=150)),
                ('origen_presupuesto', models.CharField(blank=True, max_length=200)),
                ('decision_inicial', models.CharField(blank=True, max_length=100)),
                ('numero_procedimiento', models.CharField(blank=True, max_length=120)),
                ('numero_factura', models.CharField(blank=True, max_length=120)),
                ('detalle', models.CharField(blank=True, max_length=300)),
                ('informe_tecnico', models.CharField(blank=True, max_length=120)),
                ('proveedor', models.ForeignKey(null=True, on_delete=django.db.models.deletion.SET_NULL, to='backend.proveedor')),
            ],
            options={
                'verbose_name_plural': 'Compras',
            },
        ),
        migrations.AddField(
            model_name='activos_plaqueados',
            name='categoria',
            field=models.ForeignKey(default=1, on_delete=django.db.models.deletion.DO_NOTHING, to='backend.categoria', verbose_name='Categoria'),
        ),
        migrations.AddField(
            model_name='activos_plaqueados',
            name='compra',
            field=models.ForeignKey(blank=True, null=True, on_delete=django.db.models.deletion.SET_NULL, to='backend.compra', verbose_name='Compra'),
        ),
        migrations.AddField(
            model_name='activos_plaqueados',
            name='estado',
            field=models.ForeignKey(default='1', on_delete=django.db.models.deletion.DO_NOTHING, to='backend.estados', verbose_name='Estados'),
        ),
        migrations.AddField(
            model_name='activos_plaqueados',
            name='marca',
            field=models.ForeignKey(blank=True, null=True, on_delete=django.db.models.deletion.SET_NULL, to='backend.marcas', verbose_name='Marca'),
        ),
        migrations.AddField(
            model_name='activos_plaqueados',
            name='modelo',
            field=models.ForeignKey(blank=True, null=True, on_delete=django.db.models.deletion.SET_NULL, to='backend.modelos', verbose_name='Modelo'),
        ),
        migrations.AddField(
            model_name='activos_plaqueados',
            name='partida',
            field=models.ForeignKey(default=1, on_delete=django.db.models.deletion.DO_NOTHING, to='backend.partida'),
        ),
        migrations.AddField(
            model_name='activos_plaqueados',
            name='red',
            field=models.ForeignKey(blank=True, null=True, on_delete=django.db.models.deletion.SET_NULL, to='backend.red', verbose_name='Red'),
        ),
        migrations.AddField(
            model_name='activos_plaqueados',
            name='subtipo',
            field=models.ForeignKey(blank=True, null=True, on_delete=django.db.models.deletion.SET_NULL, to='backend.subtipo', verbose_name='Subtipo'),
        ),
        migrations.AddField(
            model_name='activos_plaqueados',
            name='tipo',
            field=models.ForeignKey(null=True, on_delete=django.db.models.deletion.SET_NULL, to='backend.tipo', verbose_name='Tipo'),
        ),
        migrations.AddField(
            model_name='activos_plaqueados',
            name='ubicacion',
            field=models.ForeignKey(blank=True, null=True, on_delete=django.db.models.deletion.DO_NOTHING, to='backend.ubicaciones', verbose_name='Ubicación'),
        ),
        migrations.AddField(
            model_name='activos_plaqueados',
            name='ubicacion_anterior',
            field=models.ForeignKey(blank=True, null=True, on_delete=django.db.models.deletion.DO_NOTHING, related_name='plaqueados', to='backend.ubicaciones'),
        ),
        migrations.AddField(
            model_name='activos_no_plaqueados',
            name='categoria',
            field=models.ForeignKey(default=1, on_delete=django.db.models.deletion.DO_NOTHING, to='backend.categoria', verbose_name='Categoria'),
        ),
        migrations.AddField(
            model_name='activos_no_plaqueados',
            name='compra',
            field=models.ForeignKey(blank=True, null=True, on_delete=django.db.models.deletion.SET_NULL, to='backend.compra', verbose_name='Compra'),
        ),
        migrations.AddField(
            model_name='activos_no_plaqueados',
            name='estado',
            field=models.ForeignKey(default='1', on_delete=django.db.models.deletion.DO_NOTHING, to='backend.estados', verbose_name='Estados'),
        ),
        migrations.AddField(
            model_name='activos_no_plaqueados',
            name='marca',
            field=models.ForeignKey(blank=True, null=True, on_delete=django.db.models.deletion.SET_NULL, to='backend.marcas', verbose_name='Marca'),
        ),
        migrations.AddField(
            model_name='activos_no_plaqueados',
            name='modelo',
            field=models.ForeignKey(blank=True, null=True, on_delete=django.db.models.deletion.SET_NULL, to='backend.modelos', verbose_name='Modelo'),
        ),
        migrations.AddField(
            model_name='activos_no_plaqueados',
            name='partida',
            field=models.ForeignKey(default=1, on_delete=django.db.models.deletion.DO_NOTHING, to='backend.partida'),
        ),
        migrations.AddField(
            model_name='activos_no_plaqueados',
            name='red',
            field=models.ForeignKey(blank=True, null=True, on_delete=django.db.models.deletion.SET_NULL, to='backend.red', verbose_name='Red'),
        ),
        migrations.AddField(
            model_name='activos_no_plaqueados',
            name='subtipo',
            field=models.ForeignKey(blank=True, null=True, on_delete=django.db.models.deletion.SET_NULL, to='backend.subtipo', verbose_name='Subtipo'),
        ),
        migrations.AddField(
            model_name='activos_no_plaqueados',
            name='tipo',
            field=models.ForeignKey(null=True, on_delete=django.db.models.deletion.SET_NULL, to='backend.tipo', verbose_name='Tipo'),
        ),
        migrations.AddField(
            model_name='activos_no_plaqueados',
            name='ubicacion',
            field=models.ForeignKey(blank=True, null=True, on_delete=django.db.models.deletion.DO_NOTHING, to='backend.ubicaciones', verbose_name='Ubicación'),
        ),
        migrations.AddField(
            model_name='activos_no_plaqueados',
            name='ubicacion_anterior',
            field=models.ForeignKey(blank=True, null=True, on_delete=django.db.models.deletion.DO_NOTHING, related_name='no_plaqueados', to='backend.ubicaciones'),
        ),
    ]
